...
//int Brake_RtoD_Thold = 132;   // Brake, Ready to Drive用の閾値, とりあえず毎回調整
int Brake_Check_Thold = 133;  // Brake, Apps妥当性チェック, とりあえず毎回調整

const float min_val = 1024 * 0.1;                           //APPS,PST360-G2 出力関数：0°＝10%
const float max_val = 1024 * 0.9;                           //APPS,PST360-G2 出力関数：360°＝90%
const int deg_0 = 23;                                       //APPS,作動開始角度, 遊びを含む，ここ開始点でトルク0
const int deg_m = 38;                                       //APPS,作動限界角度，ここ終点でトルクMAX
...
void loop() {
  ...
  if (CAN.checkReceive() == CAN_MSGAVAIL) {
    ...
    //ID:801
      ...
      // 温度
      INV_deg = buf_r[0] - 40;  //インバータ温度[℃],オフセット-40，実車の正誤はまだ確認できていない(8/25)
      Mo_deg = buf_r[4] - 40;   //モータ温度[℃],オフセット-40，実車の正誤はまだ確認できていない(8/25)
      if (0) {
        Serial.print("INV:");
  ...
  if (IGNSWsta == LOW) {
    ...
    if (Op_status == B011) {  //torque control状態
      ...
      int deg_1 = map(APPS_val_1, min_val, max_val, 0, 359);  //アナログ入力値を角度に置き換え
      int deg_2 = map(APPS_val_2, min_val, max_val, 0, 359);  //アナログ入力値を角度に置き換え

      int deg_in;                                        //トルクに変換するための角度変数
      float deg_tread;                                   //APPS25％を判断するための変数
      ...
      if (deg_dev_val <= 10) {                                                       //偏差10%以下か？
        if (APPS_BK_Check == false) {                                                //前処理で25％条件に該当したか？
          if (deg_tread >= APPS_tread_threshold && Brake_val > Brake_Check_Thold) {  //同時踏みか？
            ...
            T_in = map(deg_in, deg_0, deg_m, TorMin, TorMax);
          }
      ...
      */
      ...
      if (Presta == LOW) {               //Cpu5:precharge制御は完了か？
        ...
        if (ECUsta != 1) {  //MG-ECU"OFF"なら以下を実行
          ...
          int Brake_val_RtoD = analogRead(Brake_PIN);
          if (Brake_val_RtoD >= 128) {
            RtoD_Bra = 1;
          } else {
            RtoD_Bra = 0;
          }
          int R = 0;
          if (RtoD_Bra == 1) {
            for (int r = 0; r < 350; r = r + 1) {
              RtoD = digitalRead(RtoD_PIN);
              if (RtoD == HIGH) {
                R = r;
              } else {
                R = 0;
